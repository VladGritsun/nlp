def get_class(word) :
    if any([x.isdigit() for x in word]) :
        ...
        return '_ALLCAP_'
    elif any([x.isupper() for x in word]) and word[-1].isupper() :
        return '_LASTCAP_'
    else :
        return '_RARE_'

def add_classes(in_file,out_file) :
    f_in = open(in_file)
    word_freq = {}
    ...
    for line in f_in.readlines( ) :
        ...
        if word_freq[sline[0]] < 5:
            cur_line = get_class(sline[0])+' '+sline[1]
            ...
            for v in get_tags(k) :
                ...
                if (sent[k-1],v) in emit_table :
                    ...
                    for x in tag_table :
                        ...
                        emit_val = emit_table[(get_class(sent[k-1]),v)]
...
#add_classes('gene.train','gene_mod2.train')
tagger_impr('gene.test','gene_test.p3.out.','gene_mod2.counts')
